...
def pyhop(state,tasks,verbose=0):
    ...
    """
    global executor, futures, plans
    future = executor.submit(seek_plan,state,tasks,[],0,verbose)
        ...
        for method in relevant:            
            ...
            if subtasks != False:
                if cont == 0:
                    ...
                    future = executor.submit(seek_plan,state,subtasks+tasks[1:],plan,depth+1,verbose)
                    futures.append(future)
                    #Criar Nova Thread passando usando ThreadPool
                    pass

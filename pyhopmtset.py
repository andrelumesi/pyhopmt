...
def pyhop(state,tasks,verbose=0):
    ...
    """
    thread  = threading.Thread(target=seek_plan, args=(state,tasks,[],0,verbose))
    thread.start()
        ...
        for method in relevant:            
            ...
            if subtasks != False:
                if cont == 0:
                    ...
                    global threads
                    thread  = threading.Thread(target=seek_plan, args=(state,subtasks+tasks[1:],plan,depth+1,verbose))
                    thread.start()
                    threads.append(thread)
        if firstSolution != None:
